...
const parseStatValue = (value) => {
  ...
  return { target: num, suffix: match[2] };
};
...
const AnimatedStat = ({ value, label }) => {
  ...
  return (
    <div ref={ref} className="text-center group">
      <div className="text-3xl md:text-4xl font-extrabold text-gray-900 group-hover:text-primary transition-colors">
        {inView ? display : '0'}
      </div>
      <div className="text-gray-500 font-medium text-sm mt-2 uppercase tracking-wide">{label}</div>
    </div>
  );
};
    ...
    <section className="relative bg-white overflow-hidden text-gray-900 border-b border-gray-100">
      ...
      <div className="container mx-auto px-6 py-24 md:py-32 lg:py-40 flex flex-col items-center text-center relative z-10 fade-in-up">
        ...
        <div className="flex flex-wrap justify-center gap-12 mt-20 pt-10 border-t border-gray-100 w-full max-w-3xl">
          <AnimatedStat value="10000+" label="Career Paths" />
          <AnimatedStat value="97%" label="Accuracy Rate" />
          <AnimatedStat value="50+" label="Industries" />
        </div>
      </div>
    </section>
  );
